...
//    }
    ...
    public function rules() {
        ...
        return array(
            ...
            array('is_allow_login,prefix,is_cashier', 'numerical', 'integerOnly' => true),
    ...
    public function search($type = 'user') {
        ...
        $criteria->compare('siteconfig_id', user()->siteconfig_id);
...
//           $criteria->compare('id', $this->id);
            ...
            $criteria->alias = "u";
            $siteConfig = SiteConfig::model()->listSiteConfig();
            $sCriteria = json_decode($siteConfig->roles_member, true);
            $criteria->addInCondition('id', $sCriteria);
    ...
    public function listRole($type = '') {
        ...
        if ($type == 'supplier') {
            ...
            $sResult = Roles::model()->findAll(array('condition' => 'siteconfig_id='.user()->siteconfig_id.' and id in(' . $list . ')'));
            }else{
                $sResult='';
            }
        }elseif ($type == 'vendor') {
            ...
            $sResult = Roles::model()->findAll(array('condition' => 'siteconfig_id='.user()->siteconfig_id.' and is_allow_login=1'));
        } elseif ($type == 'employment') {
            $sCriteria = json_decode($siteConfig->roles_employment, true);
            if (!empty($sCriteria)) {
                $list = '';
    ...
    public function getCashier() {
        $status = ($this->is_cashier == 0) ? "<span class=\"label label-important\">No</span>" :
                "<span class=\"label label-info\">Yes</span>";
        return $status;
    }
