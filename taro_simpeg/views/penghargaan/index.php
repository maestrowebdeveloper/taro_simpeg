<?php
$this->setPageTitle('Penghargaans');
...
$('.search-form form').submit(function(){
    $.fn.yiiGridView.update('penghargaan-grid', {
        data: $(this).serialize()
    });
    return false;
});
...
<?php
	$display = (landa()->checkAccess("penghargaan","d")==0)?'none':'';
	$button = "";
    if (landa()->checkAccess("penghargaan", 'r')) 
        $button .= '{view} ';    
    if (landa()->checkAccess("penghargaan", 'u')) 
        $button .= '{update} ';    
    if (landa()->checkAccess("penghargaan", 'd')) 
        $button .= '{delete}';    
