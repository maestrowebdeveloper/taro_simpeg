<?php
$this->setPageTitle('Jabatan Honorers');
...
$('.search-form form').submit(function(){
    $.fn.yiiGridView.update('jabatan-honorer-grid', {
        data: $(this).serialize()
    });
    return false;
});
...
<?php
	$display = (landa()->checkAccess("jabatanHonorer","d")==0)?'none':'';
	$button = "";
    if (landa()->checkAccess("jabatanHonorer", 'r')) 
        $button .= '{view} ';    
    if (landa()->checkAccess("jabatanHonorer", 'u')) 
        $button .= '{update} ';    
    if (landa()->checkAccess("jabatanHonorer", 'd')) 
        $button .= '{delete}';    
