<?php
if (isset($_GET['v'])) {$this->setPageTitle('Lihat Permohonan Pensiuns | ID : '. $model->id);
$this->breadcrumbs=array(
	'Permohonan Pensiuns'=>array('index'),
	$model->id,
);
}else{$this->setPageTitle('Edit Permohonan Pensiuns | ID : '. $model->id);
$this->breadcrumbs=array(
	'Permohonan Pensiuns'=>array('index'),
...
if (isset($_GET['v'])) {
	...
	'items'=>array(
		array('label'=>'Tambah', 'icon'=>'icon-plus', 'url'=>Yii::app()->controller->createUrl('create'), 'linkOptions'=>array(),'visible'=>landa()->checkAccess('permohonanPensiun', 'c')),
                ...
                array('label'=>'Edit', 'icon'=>'icon-edit', 'url'=>Yii::app()->controller->createUrl('update',array('id'=>$model->id)), 'linkOptions'=>array(),'visible'=>landa()->checkAccess('permohonanPensiun', 'u')),
