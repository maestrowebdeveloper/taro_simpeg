<?php
if (isset($_GET['v'])) {$this->setPageTitle('Lihat Transfer Cpns | ID : '. $model->id);
$this->breadcrumbs=array(
	'Transfer Cpns'=>array('index'),
	$model->id,
);
}else{$this->setPageTitle('Edit Transfer Cpns | ID : '. $model->id);
$this->breadcrumbs=array(
	'Transfer Cpns'=>array('index'),
...
if (isset($_GET['v'])) {
	...
	'items'=>array(
		array('label'=>'Tambah', 'icon'=>'icon-plus', 'url'=>Yii::app()->controller->createUrl('create'), 'linkOptions'=>array(),'visible'=>landa()->checkAccess('transferCpns', 'c')),
                ...
                array('label'=>'Edit', 'icon'=>'icon-edit', 'url'=>Yii::app()->controller->createUrl('update',array('id'=>$model->id)), 'linkOptions'=>array(),'visible'=>landa()->checkAccess('transferCpns', 'u')),
