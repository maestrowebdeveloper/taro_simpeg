...
$('.search-form form').submit(function(){
    $.fn.yiiGridView.update('jabatan-struktural-grid', {
        data: $(this).serialize()
    });
    return false;
});
...
$this->widget('bootstrap.widgets.TbMenu', array(
	'type'=>'pills',
	'items'=>array(
		array('label'=>'Tambah', 'icon'=>'icon-plus', 'url'=>Yii::app()->controller->createUrl('create'), 'linkOptions'=>array(),'visible'=>landa()->checkAccess('jabatanStruktural', 'c')),
...
<?php
	$display = (landa()->checkAccess("jabatanStruktural","d")==0)?'none':'';
	$button = "";
    if (landa()->checkAccess("jabatanStruktural", 'r')) 
        $button .= '{view} ';    
    if (landa()->checkAccess("jabatanStruktural", 'u')) 
        $button .= '{update} ';    
    if (landa()->checkAccess("jabatanStruktural", 'd')) 
        $button .= '{delete}';    
