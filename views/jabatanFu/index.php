<?php
$this->setPageTitle('Jabatan Fungsional Umum');
...
$('.search-form form').submit(function(){
    $.fn.yiiGridView.update('jabatan-fu-grid', {
        data: $(this).serialize()
    });
    return false;
});
...
<?php
	$display = (landa()->checkAccess("jabatanFu","d")==0)?'none':'';
	$button = "";
    if (landa()->checkAccess("jabatanFu", 'r')) 
        $button .= '{view} ';    
    if (landa()->checkAccess("jabatanFu", 'u')) 
        $button .= '{update} ';    
    if (landa()->checkAccess("jabatanFu", 'd')) 
        $button .= '{delete}';    
